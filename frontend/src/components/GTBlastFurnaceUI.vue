...
const machineLabel = computed(() => {
  ...
  return recipeData.value.recipeType || 'Blast Furnace';
});
...
const requiredHeat = computed(() => pickNumber(
  mergedMeta.value.heat,
  mergedMeta.value.heatRequired,
  mergedMeta.value.requiredHeat,
  mergedMeta.value.minHeat,
) ?? null);
...
<template>
  <div class="gt-machine-ui gt-heat-ui">
    ...
    <div class="recipe-row">
      ...
      <section class="core-panel">
        <div class="heat-chip">Heat{{ requiredHeat !== null ? ` · ${requiredHeat.toLocaleString()}K` : '' }}</div>
...
.gt-heat-ui {
  background:
    linear-gradient(180deg, rgba(18, 11, 8, 0.95), rgba(10, 8, 6, 0.98)),
    radial-gradient(circle at top, rgba(251, 146, 60, 0.12), transparent 42%);
}
...
.machine-header {
  ...
  background:
    linear-gradient(180deg, rgba(47, 31, 22, 0.94), rgba(28, 19, 14, 0.98)),
...
.recipe-row {
  ...
  background:
    linear-gradient(180deg, rgba(22, 13, 9, 0.88), rgba(12, 9, 7, 0.94)),
...
.core-panel {
  ...
  border-radius: 12px;
  background:
    linear-gradient(180deg, rgba(39, 25, 18, 0.94), rgba(20, 14, 11, 0.98)),
...
.gt-slot {
  ...
  background:
    linear-gradient(180deg, rgba(28, 16, 10, 0.9), rgba(14, 10, 8, 0.94)),
...
.gt-slot::before {
  ...
  background: radial-gradient(circle at 30% 25%, rgba(255, 255, 255, 0.12), transparent 48%);
  opacity: 0.45;
  pointer-events: none;
}
...
.gt-slot:hover {
  ...
  box-shadow:
    ...
    0 10px 18px rgba(154, 52, 18, 0.24);
}
