...
const machineLabel = computed(() => {
  ...
  return recipeData.value.recipeType || 'Electric Furnace';
});
...
const initMachine = async () => {
  const [firstInput] = await buildInputSlots(recipeData.value);
        ...
        >
          <div class="gt-slot">
            <AnimatedItemIcon
              ...
              class="item-icon"
            />
            <span v-if="inputSlot.count > 1" class="item-count">{{ inputSlot.count }}</span>
          </div>
        </RecipeItemTooltip>
